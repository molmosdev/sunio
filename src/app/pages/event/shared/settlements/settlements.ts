  ...
  template: `
    @if (isSettlementsLoading()) {
      ...
      @if (mySuggestedSettlements().length > 0) {
        ...
        <span class="text-center">{{ 'event.settlements.all-settled' | translate }}</span>
      }
      <button
        b-button
      ...
      <ng-template #allSettlementsTpl>
        <b-tabs class="b-rounded-full flex-1" [(value)]="selectedAllSettlementsTab">
          <b-tab value="old-mine" class="flex-1 b-rounded-full">
            {{ 'event.settlements.settled' | translate }}
          </b-tab>
          <b-tab value="others" class="flex-1 b-rounded-full">
            {{ 'event.participants.title' | translate }}
          </b-tab>
        </b-tabs>
        @if (selectedAllSettlementsTab()[0] === 'old-mine') {
          @if (myPastSettlements().length === 0) {
            <span class="text-center my-2">{{ 'event.settlements.empty-past' | translate }}</span>
          } @else {
          ...
          @if (othersSuggestedSettlements().length === 0) {
            <span class="text-center my-2">{{ 'event.settlements.empty-others' | translate }}</span>
          } @else {
      ...
      <ng-template #settlementOptionsTpl>
        <button
          ...
          (click)="onPaySettlementButtonClicked()"
        >
          @if (selectedSettlement()?.payment_id) {
            {{ 'event.settlements.undo-payment' | translate }}
          } @else {
            {{ 'event.settlements.register-payment' | translate }}
          }
        </button>
      </ng-template>
    }
